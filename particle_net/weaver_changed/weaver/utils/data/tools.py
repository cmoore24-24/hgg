...
def _clip(a, a_min, a_max):
    if isinstance(a, np.ndarray) or a.ndim == 1:
        ...
        try:
            arr = ak.unflatten(np.clip(ak.to_numpy(ak.flatten(a)), a_min, a_max), ak.num(a))
        except:
            arr = ak.unflatten(np.clip(ak.to_numpy(ak.flatten(ak.flatten(a))), a_min, a_max), ak.num(ak.flatten(a)))
        return arr
