...
def _main(args):
    ...
    if training_mode:
        ...
        for epoch in range(args.num_epochs):
            ...
            if args.model_prefix and (args.backend is None or local_rank == 0):
                ...
                # traced_model_path = args.model_prefix + '_best_epoch_traced.pt'
                # example_input = tuple(
                #     torch.randn((args.batch_size,) + model_info['input_shapes'][k][1:],
                #                 dtype=torch.float32, device=dev)
                #     for k in model_info['input_names']
                # )
                # traced_model = torch.jit.trace(orig_model, example_input)
    ...
    if args.data_test:
        ...
        if not args.model_prefix.endswith('.onnx'):
            ...
            # Save a traced version of the model for inference
            # traced_model_path = args.model_prefix.replace('.pt', '_traced.pt')
            # example_input = tuple(
            #     torch.randn((args.batch_size,) + model_info['input_shapes'][k][1:],
            #                 dtype=torch.float32, device=dev)
            #     for k in model_info['input_names']
