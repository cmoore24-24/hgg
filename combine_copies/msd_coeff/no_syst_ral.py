...
def test_rhalphabet(tmpdir):
    ...
    tf_MCtempl = rl.BasisPoly("tf_MCtempl", (0,), ["msd"], basis="Bernstein", limits=(0, 10))
    ...
    tf_dataResidual = rl.BasisPoly("tf_dataResidual", (0,), ["msd"], basis="Bernstein", limits=(0, 10))
    ...
    for ptbin in range(npt):
        for region in ["pass", "fail"]:
            ...
            # }
            with open('sample_hist.pkl','rb') as f:
                templates = pickle.load(f)
                
            for sName in ["hgg"]:
                # some mock expectations
                templ = templates[f'{sName}_pass_nominal_2017']
            ...
            # }
            yields = sum(tpl[0] for tpl in templates.values() if 'qcd' in templates and 'nominal' in templates)
            if throwPoisson:
