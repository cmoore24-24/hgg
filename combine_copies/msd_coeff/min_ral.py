...
def smass(sName):
    if 'hbb' in sName or 'hww' in sName or 'gg' in sName:
        _mass = 125.
    elif sName in ['wqq', 'ww', 'ttboosted', 'singletop']:
        _mass = 80.379
    elif sName in ['zqq', 'zbb','zz']:
        _mass = 91.
    else:
...
def get_templ(f, region, sample, ptbin, syst=None, muon=False, nowarn=False):
    if "16" in list(f.keys())[0]:
        year = 2016
    elif "17" in list(f.keys())[0]:
        year = 2017
    else:
        year = 2018
    ...
    if syst is not None:
        hist_name += f"_{syst}_{year}"
    else:
        hist_name += f"_nominal_{year}"
    # if not muon:
...
def dummy_rhalphabet(pseudo,
    ...
    sys_shape_dict['jet_trigger'] = rl.NuisanceParameter('CMS_gghgg_trigger_{}'.format(year), sys_types['jet_trigger'])
    ...
    sys_shape_dict['pileup_weight'] = rl.NuisanceParameter('CMS_gghgg_PU_{}'.format(year), sys_types['pileup_weight'])
    # sys_shape_dict['HEM18'] = rl.NuisanceParameter('CMS_gghcc_HEM_{}'.format(year), sys_types['HEM18'])
    sys_shape_dict['L1Prefiring'] = rl.NuisanceParameter('CMS_gghgg_L1prefire_{}'.format(year), sys_types['L1Prefiring'])
    ...
    for sys in [ 'btagWeight', #'btagEffStat'
               ]:
        sys_shape_dict[sys] = rl.NuisanceParameter('CMS_gghgg_{}_{}'.format(sys, year), sys_types[sys])
    # for sys in ['d1kappa_EW', 'Z_d2kappa_EW', 'Z_d3kappa_EW', 'd1K_NLO', 'd2K_NLO', 'd3K_NLO']:
        # sys_shape_dict[sys] = rl.NuisanceParameter('CMS_gghgg_{}'.format(sys), sys_types[sys])


    sys_eleveto = rl.NuisanceParameter('CMS_gghgg_e_veto_{}'.format(year), 'lnN')
    sys_muveto = rl.NuisanceParameter('CMS_gghgg_m_veto_{}'.format(year), 'lnN')
    sys_tauveto = rl.NuisanceParameter('CMS_gghgg_tau_veto_{}'.format(year), 'lnN')

    sys_wznormEW = rl.NuisanceParameter('CMS_gghgg_wznormEW', 'lnN')
    sys_znormEW = rl.NuisanceParameter('CMS_gghgg_znormEW', 'lnN')
    sys_znormQ = rl.NuisanceParameter('CMS_gghgg_znormQ', 'lnN')

    sys_veff = rl.NuisanceParameter('CMS_gghgg_veff_{}'.format(year), 'lnN')
    sys_veff_indept = {}
    for veff in ["h", "w", "z"]:
        sys_veff_indept['sys_veff_{}'.format(veff)] = rl.NuisanceParameter('CMS_gghgg_veff_{}_{}'.format(veff, year), 'lnN')
    sys_scale = rl.NuisanceParameter('CMS_gghgg_scale_{}'.format(year), 'shape')
    # Independent terms
    sys_scaleW = rl.NuisanceParameter('CMS_gghgg_scaleW_{}'.format(year), 'shape')
    sys_scaleZ = rl.NuisanceParameter('CMS_gghgg_scaleZ_{}'.format(year), 'shape')
    sys_scaleHbb = rl.NuisanceParameter('CMS_gghgg_scaleHbb_{}'.format(year), 'lnN')
    sys_scale_indept = {}
    for i in range(6):
        sys_scale_indept['sys_scale'+str(i)] = rl.NuisanceParameter('CMS_gghgg_scale_PT{}_{}'.format(i, year), 'shape')
    # sys_smear = rl.NuisanceParameter('CMS_gghgg_smear_{}'.format(year), 'shape')

    from config_Hxx import ddxSF, get_bins
    msdbins, ptbins, msdscaled, ptscaled, rhoscaled = get_bins(year=args.year)
    ...
    if opts.templates is not None:
        ...
        with open(opts.templates, 'rb') as file:
            f = pickle.load(file)
    ...
    if MCTF:
        ...
        print(f'THE QCD STATUS IS {qcdfit.status()}')
    ...
    for ptbin in range(npt):
        for region in ['pass', 'fail']:
            ...
            for sName in include_samples:
                ...
                def smorph(templ, sName, MORPHNOMINAL=False):
                    if templ is None:
                        return None
                    if MORPHNOMINAL and sName not in ['qcd','ww','wz','zz']:
                        return MorphHistW2(templ).get(shift=SF[year]['shift_SF']/smass('wqq') * smass(sName),
                                                      smear=SF[year]['smear_SF']
                                                      )
                    else:
                        return templ
                # templ = smorph(templ, sName, MORPHNOMINAL)
                ...
                if not systs:  # Need at least one
                    ...
                    sys_names = [
                        # 'JES', 'JER', 'UES', 'jet_trigger',  'btagWeight', 'pileup_weight',
                    ...
                    for sys_name in sys_names:
                        ...
                        if ("Z_d" in sys_name) and sName not in ['zqq']:
                            continue
                        if sys_name == 'scalevar_7pt' and sName not in ['hww', 'hbb', 'hgg']:
                            continue
                        ...
                        if sys_shape_dict[sys_name].combinePrior == "lnN":
                            ...
                            _dn = get_templ(f,
                                            ...
                                            ptbin,
                                            syst=sys_name + "Down")
                            _up = smorph(_up, sName, MORPHNOMINAL)
                            _dn = smorph(_dn, sName, MORPHNOMINAL)
                            ...
                            sample.setParamEffect(sys_shape_dict[sys_name],
                                _up[:-1], _dn[:-1])
                    # One sided
                    # for sys_name in ['HEM18']: #Commented out myself
                    #     if sys_shape_dict[sys_name].combinePrior == "lnN":
                    #         _sys_ef = shape_to_num(f, region, sName, ptbin,
                    #                                 sys_name, mask)
                    #         if _sys_ef is None:
            ...
            if not pseudo:
                ...
                for sName in include_samples:
                    if sName == "qcd" and opts.mockQCD and region == "pass":
                        ...
                        _sample_yield = smorph(get_templ(f, region, sName, ptbin), sName, True)[0] #Not sure if this is correct

                    # if sName not in ["qcd",] and systs:
                    #     _sample_yield *= SF[year]['V_SF']
                    # if sName in ["hgg"]:
                    #     _sf, _sfup, _sfdn = ddxSF(ptbin, 'gg')
                    #     _sf, _sfunc = passfailSF(f, region, sName, ptbin, mask,
                    #                              _sf,
                    #                              _sfdn)
                    #     _sample_yield *= _sf
    ...
    if fitTF:
        ...
        degs = tuple([int(opts.degs),])
        print(f'Second degs: {degs}')
        ...
        tf_dataResidual = rl.BasisPoly("tf{}_dataResidual".format(year),
                                       degs, ['msd'], basis=_basis, init_params=_inits,
