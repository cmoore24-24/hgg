...
def test_rhalphabet(tmpdir):
    ...
    jec = rl.NuisanceParameter("CMS_jec", "lnN") # change to shape
    ...
    ptscaled = (ptpts - 475.0) / (1000.0 - 475.0)
    ...
    for ptbin in range(npt):
        ...
        failTempl = load_hist('hgg_vs_qcd_fail_ecf_md_pn_28.pkl')
        passTempl = load_hist('hgg_vs_qcd_pass_ecf_md_pn_28.pkl')
    ...
    tf_MCtempl = rl.BernsteinPoly("tf_MCtempl", (3,), ["msd"], limits=(0, 10))
    ...
    for ptbin in range(npt):
        for region in ["pass", "fail"]:
            ...
            ptnorm = 1.0
            templates = {
                "wqq": load_hist(f'hgg_vs_wqq_{region}_ecf_md_pn_28.pkl'),
                "zqq": load_hist(f'hgg_vs_zqq_{region}_ecf_md_pn_28.pkl'),
                "singletop": load_hist(f'hgg_vs_singletop_{region}_ecf_md_pn_28.pkl'),
                "hgg": load_hist(f'hgg_{region}_ecf_md_pn_28.pkl'),
                "hbb": load_hist(f'hgg_vs_hbb_{region}_ecf_md_pn_28.pkl'),
                "ttboosted": load_hist(f'hgg_vs_ttboosted_{region}_ecf_md_pn_28.pkl'),
                "ww": load_hist(f'hgg_vs_ww_{region}_ecf_md_pn_28.pkl'),
                "wz": load_hist(f'hgg_vs_wz_{region}_ecf_md_pn_28.pkl'),
                "zz": load_hist(f'hgg_vs_zz_{region}_ecf_md_pn_28.pkl'),
                "hww": load_hist(f'hgg_vs_hww_{region}_ecf_md_pn_28.pkl'),
            }
            for sName in ["zqq", "wqq", "singletop", "hgg", "hbb", "ttboosted", "ww", "wz", "zz", "hww"]: 
            ...
            yields += load_hist(f'./hgg_vs_qcd_{region}_ecf_md_pn_28.pkl').values()
            if throwPoisson:
