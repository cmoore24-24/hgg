...
def smass(sName):
    if 'hbb' in sName or 'hww' in sName or 'hgg' in sName:
        _mass = 125.
...
def dummy_rhalphabet(pseudo,
    ...
    sys_ddxeff = rl.NuisanceParameter('CMS_eff_gg_{}'.format(year), 'lnN')
        ...
        for region in ['pass', 'fail']:
            ...
            for sName in include_samples:
                ...
                def smorph(templ, sName, MORPHNOMINAL=False):
                    if templ is None:
                        return None
                    if MORPHNOMINAL and sName not in ['qcd', 'zll', 'wln', 'ww','wz','zz']:
