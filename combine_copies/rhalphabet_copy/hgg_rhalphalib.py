...
def load_hist(filename):
    with open(f'hists/hgg_hists/{filename}', 'rb') as f:
        hist = pickle.load(f)
    return hist
...
def test_rhalphabet(tmpdir):
    ...
    for ptbin in range(npt):
        ...
        failTempl = load_hist('./hgg_vs_qcd_fail_wp_neg1.pkl')
        passTempl = load_hist('./hgg_vs_qcd_pass_wp_neg1.pkl')
    ...
    for ptbin in range(npt):
        for region in ["pass", "fail"]:
            ...
            ptnorm = 1.0
            templates = {
                "wqq": load_hist(f'./hgg_vs_wqq_{region}_wp_neg1.pkl'),
                "zqq": load_hist(f'./hgg_vs_zqq_{region}_wp_neg1.pkl'),
                "singletop": load_hist(f'./hgg_vs_singletop_{region}_wp_neg1.pkl'),
                "hgg": load_hist(f'./hgg_{region}_wp_neg1.pkl'),
                "hbb": load_hist(f'./hgg_vs_hbb_{region}_wp_neg1.pkl'),
                "ttboosted": load_hist(f'./hgg_vs_ttboosted_{region}_wp_neg1.pkl'),
                "ww": load_hist(f'./hgg_vs_ww_{region}_wp_neg1.pkl'),
                "wz": load_hist(f'./hgg_vs_wz_{region}_wp_neg1.pkl'),
                "zz": load_hist(f'./hgg_vs_zz_{region}_wp_neg1.pkl'),
            }
            ...
            yields += load_hist(f'./hgg_vs_qcd_{region}_wp_neg1.pkl').values()
            if throwPoisson:
...
if __name__ == "__main__":
    if not os.path.exists("hgg_out"):
        os.mkdir("hgg_out")
    test_rhalphabet("hgg_out")
